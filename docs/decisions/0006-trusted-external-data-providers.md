...
- 状态：Accepted；简报执行方式由 ADR 0007 修订；2026-08-09 增补国内天气路由
- 日期：2026-07-19
...
8. 配置 `AMAP_WEATHER_ENABLED=true` 且存在高德 Web Service Key 时，中国境内天气优先使用高德行政区预报；只发送六位 `adcode`，不为天气查询向高德发送用户 GPS。无法从已确认地址得到 `adcode` 或高德调用失败时，降级到 Open-Meteo。
9. 新闻 Feed 必须在目标部署网络中同时通过可达性和 RSS/Atom 解析验证。长期不可达或返回 HTML 的地址不得继续留在生产目录中制造永久告警；替换来源仍须是明确登记的可信 HTTPS Feed。

## 结果

用户通过少量主题即可影响可信来源的选择和排序，运维方可以审计或替换 Provider。国内天气在保护精确坐标的前提下使用更适合行政区的高德预报，并保留 Open-Meteo 降级路径。目录标签未覆盖的主题可能通过可信目录关键词检索得到结果，但不会扩展到任意网页；新增来源仍需要显式配置和验证。
