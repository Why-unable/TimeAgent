...
7. 高风险注册表只包含可真实执行且有测试覆盖的 Tool。日历创建/修改类操作可按 Schema 支持 approve/edit/reject；取消类操作以及长期记忆的 remember/update/forget 只允许 approve/reject，避免审批时替换撤销目标或记忆内容。未来加入物理删除、批量或外部操作时，仍须先实现对应 Service、Tool、风险策略和测试。
...
## 2026-08-31 扩展：语义记忆写 Tool

语义记忆写 Tool 复用本 ADR 的同一中断和恢复边界。ActionProposal 的
`display_context` 额外保存目标 `SemanticMemory` 的 ID、版本和值快照；恢复执行时
`SemanticMemoryToolService` 必须验证当前用户、AgentRun、真实 Tool Call ID、批准状态和目标
版本。验证失败时审批标记为 failed 且不创建 MemoryProposal。验证成功后仍通过 MemoryPolicy
创建并应用 MemoryProposal，ActionProposal 不能成为第二个记忆写入口。
