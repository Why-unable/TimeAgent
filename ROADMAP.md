...
> 本文记录 Phase 0 至 Phase 11 的交付路线和当前实施状态。Phase 10 之后的 AI 原生产品定位、
> 智能闭环、自治边界和未来阶段规划，见
> [Time Agent：AI 原生产品战略与未来演进路线](docs/product/ai-native-time-agent-strategy.md)。
> 新文档不改变本文已经记录的完成状态，也不覆盖现有 ADR。

当前进度：Phase 0 至 Phase 10 已完成；Phase 11 正在进行。

阶段状态采用两级验收口径：Phase 10 已完成的是生产工程与发布基线；必须依赖真实账号、
真实设备、真实流量或长期运行数据的生产实证，不反向阻塞 Phase 10，而是作为 Phase 11 的
独立交付项。详细审查证据见
[项目状态审查（2026-09-16）](docs/product/project-status-review-2026-09-16.md)。
...
- 后端：账户认证、结构化日志、完整观测栈、备份恢复、Agent 评测与安全、长期时间记忆、同用户日程写串行化和游客隔离。
- 前端：生产构建、错误关联、缓存策略、响应式完善、首次引导、时间记忆管理和 Android 自托管更新入口。
- 测试：本地单元/组件/静态检查、容器集成、代理与更新发布校验、生产健康检查和安全配置检查。
- 验收：具备 TLS、监控、告警配置、备份恢复工具与文档、可重复部署流程，以及可验证的 Web/Android 发布产物。
- 不包含：Kubernetes、微服务拆分、复杂多租户。

Phase 10 已交付账户体系、生产 Compose/Cloudflare 基线、结构化日志与观测栈、备份恢复工具、
长期时间记忆、Agent 安全、同用户写串行化、双坐标天气、隔离游客空间和 Android 自托管
更新链路。Web 已部署，Android `1.1.8 / versionCode 12` 已生成、签名并发布。需要真实外部
环境或持续运行才能取得的证据统一进入 Phase 11。

## Phase 11：生产验证、质量闭环与工具集治理

- 状态：进行中。
- 目标：用真实环境证据验证 Phase 10 的工程能力，并降低 Agent 工具调用成本与批量写入风险。
- 外部链路：Google Calendar 专用沙箱、SMTP/Web Push、天气 Provider 和告警送达实测。
- 可靠性：隔离数据库恢复与回滚演练，API/SSE/Celery/Agent 分层负载及公开入口安全基线。
- 客户端：Android `1.1.8` 真机升级、离线恢复、进程被杀和失败反馈矩阵。
- Agent：真实模型发布评测、扩展轨迹集、44 Tool Manifest/风险策略一致性、批量排程路径和分段性能指标。
- 产品效果：采集匿名化计划采纳、修改、撤销、完成和估时偏差数据，验证用户价值而非只验证功能存在。
- 验收：每项均有可重复命令、时间戳、环境说明、脱敏结果和明确通过/失败结论；未取得证据的项目不得写成已验证。
- 不包含：Microsoft Calendar、Webhook、外部日历写回、应用商店发布、Telegram/SMS、Kubernetes、微服务、向量数据库和复杂 RBAC；这些仍属于后续独立范围。
...
| T085 | 完成生产安全响应头、Compose 健康恢复与部署验收（已完成） | Phase 10 |
| T086 | 实现基于业务事实的 Time Steward 确定性长期记忆与用户控制（已完成） | Phase 10 |
| T087 | 实现 Android 自托管更新清单、原生校验与安装确认（已完成） | Phase 10 |
| T088 | 扩展 Grafana/Alertmanager/Loki/Alloy、LLM 审计、发布评测与 Agent 安全（已完成） | Phase 10 |
| T089 | 实现同用户日程写事务级串行化（已完成） | Phase 10 |
| T090 | 实现行政区代表点与设备 GPS 双坐标天气简报（已完成） | Phase 10 |
| T091 | 实现隔离、限额且自动过期的游客工作空间与首次引导（已完成） | Phase 10 |
| T092 | 完成 Google Calendar 专用沙箱授权、撤权、限流和长期增量同步验收 | Phase 11 |
| T093 | 完成 SMTP/Web Push、天气 Provider 与 Alertmanager 真实送达验收 | Phase 11 |
| T094 | 完成 Android 1.1.8 真机升级、离线、进程恢复与失败反馈矩阵 | Phase 11 |
| T095 | 完成隔离 PostgreSQL 恢复、应用回滚和灾难恢复演练 | Phase 11 |
| T096 | 建立 API、SSE、Celery、Agent 分层负载与公开入口安全基线 | Phase 11 |
| T097 | 扩展真实模型评测集并将轨迹、质量、成本和延迟纳入发布门禁 | Phase 11 |
| T098 | 落地 Tool Manifest、风险策略一致性、批量排程与版本幂等加固 | Phase 11 |
| T099 | 建立计划采纳、修改、撤销、完成和估时偏差的产品效果指标 | Phase 11 |
