...
- 测试：本地单元/组件/静态检查、容器集成、真实模型发布评测、代理与更新回滚、告警送达、备份恢复、基础负载和安全检查。
- 验收：具备 TLS、监控、告警、备份与可重复部署流程。
- 不包含：Kubernetes、微服务拆分、复杂多租户。

当前代码与本地测试已覆盖长期时间记忆、生产观测与 Agent 安全、同用户写串行化、
双坐标天气、隔离游客空间和 Android 更新清单。Phase 10 仍保持“进行中”，直到真实模型、
外部 Provider、完整容器观测栈、告警送达、数据库恢复和 Android 安装链路取得运行验收证据。
...
| T086 | 实现基于业务事实的 Time Steward 确定性长期记忆与用户控制（本地检查已完成） | Phase 10 |
| T087 | 实现 Android 自托管更新清单、原生校验与安装确认（运行验收中） | Phase 10 |
| T088 | 扩展 Grafana/Alertmanager/Loki/Alloy、LLM 审计、发布评测与 Agent 安全（运行验收中） | Phase 10 |
| T089 | 实现同用户日程写事务级串行化（本地检查已完成） | Phase 10 |
| T090 | 实现行政区代表点与设备 GPS 双坐标天气简报（本地检查已完成） | Phase 10 |
| T091 | 实现隔离、限额且自动过期的游客工作空间与首次引导（本地检查已完成） | Phase 10 |
